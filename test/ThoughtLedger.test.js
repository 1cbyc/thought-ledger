...
describe("ThoughtLedger", function () {
  ...
  function getThoughtPostedEvent(receipt, contract) {
    const eventSig = contract.interface.getEvent("ThoughtPosted").topicHash;
    const log = receipt.logs.find(l => l.topics[0] === eventSig);
    if (!log) return undefined;
    return contract.interface.decodeEventLog("ThoughtPosted", log.data, log.topics);
  }
  ...
  describe("Posting Thoughts", function () {
    it("Should allow posting a thought with correct payment", async function () {
      ...
      const tx = await thoughtLedger.connect(user1).postThought(name, message, { value: postingFee });
      const receipt = await tx.wait();
      const after = await time();
      const event = getThoughtPostedEvent(receipt, thoughtLedger);
      expect(event).to.not.be.undefined;
      expect(event.sender).to.equal(user1.address);
      expect(event.name).to.equal(name);
      expect(event.message).to.equal(message);
      expect(Number(event.timestamp)).to.be.within(before, after);
      expect(Number(event.thoughtId)).to.equal(0);
      const thoughts = await thoughtLedger.getAllThoughts();
      expect(thoughts.length).to.equal(1);
      expect(Number(thoughts[0].timestamp)).to.be.within(before, after);
    });
